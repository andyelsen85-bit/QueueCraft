...
COPY deploy/docker/watch-certificates.sh /usr/local/bin/queuecraft-watch-certificates
COPY --from=builder /app/artifacts/queuecraft/dist/public /usr/share/nginx/html
RUN chmod +x /usr/local/bin/queuecraft-web-entrypoint /usr/local/bin/queuecraft-watch-certificates \
  && chown -R nginx:nginx /usr/share/nginx/html \
  && chown nginx:nginx /etc/nginx/nginx.conf /etc/nginx/tls-listen.conf \
    /usr/local/bin/queuecraft-web-entrypoint /usr/local/bin/queuecraft-watch-certificates
USER nginx
EXPOSE 80 443
