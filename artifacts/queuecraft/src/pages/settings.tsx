...
type Status = {
  ...
  adfs: { issuer: string | null; clientId: string | null; clientSecretConfigured: boolean; redirectUri: string | null }
  ldaps: { url: string | null; bindDn: string | null; bindPasswordConfigured: boolean; baseDn: string | null; userFilter: string; cioGroupDn: string | null; caCertificateConfigured: boolean }
...
export function SettingsPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    setStatus(next)
    setDraft({
      publicBaseUrl: next.publicBaseUrl ?? "", adfsIssuer: next.adfs.issuer ?? "", adfsClientId: next.adfs.clientId ?? "", adfsRedirectUri: next.adfs.redirectUri ?? "",
  ...
  return <div className="flex-1 space-y-6 p-8">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Section icon={ShieldCheck} title="AD FS / OpenID Connect"><Text label="Issuer URL" value={draft.adfsIssuer} onChange={(v) => set("adfsIssuer", v)} /><Text label="Client ID" value={draft.adfsClientId} onChange={(v) => set("adfsClientId", v)} /><Secret label="Client secret" configured={status.adfs.clientSecretConfigured} onChange={(v) => set("adfsClientSecret", v)} /><Text label="Redirect URI" value={draft.adfsRedirectUri} onChange={(v) => set("adfsRedirectUri", v)} /></Section>
      <Section icon={Network} title="LDAPS / Active Directory"><Text label="LDAPS URL" value={draft.ldapsUrl} onChange={(v) => set("ldapsUrl", v)} /><Text label="Bind DN" value={draft.ldapsBindDn} onChange={(v) => set("ldapsBindDn", v)} /><Secret label="Bind password" configured={status.ldaps.bindPasswordConfigured} onChange={(v) => set("ldapsBindPassword", v)} /><Text label="Base DN" value={draft.ldapsBaseDn} onChange={(v) => set("ldapsBaseDn", v)} /><Text label="User filter" value={draft.ldapsUserFilter} onChange={(v) => set("ldapsUserFilter", v)} /><Text label="CIO group DN" value={draft.ldapsCioGroupDn} onChange={(v) => set("ldapsCioGroupDn", v)} /><Secret label="CA certificate (PEM)" configured={status.ldaps.caCertificateConfigured} onChange={(v) => set("ldapsCaCertificate", v)} /></Section>
