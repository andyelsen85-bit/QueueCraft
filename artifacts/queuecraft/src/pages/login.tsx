  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-6">
      <div className="w-full max-w-md space-y-6 rounded-lg border bg-card p-8 shadow-sm">
        ...
        {!bootstrap && ldaps && <div className="grid grid-cols-2 gap-2"><Button type="button" variant={method === "ldaps" ? "default" : "outline"} onClick={() => setMethod("ldaps")}>LDAPS account</Button><Button type="button" variant={method === "local" ? "default" : "outline"} onClick={() => setMethod("local")}>Local administrator</Button></div>}
        {!bootstrap && <div className="space-y-3"><Label>{method === "ldaps" ? "Directory username" : "Username"}</Label><Input value={username} onChange={(e) => setUsername(e.target.value)} /></div>}
