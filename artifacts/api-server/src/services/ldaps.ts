...
export async function authenticateWithLdaps(username: string, password: string) {
  ...
  const ldap = {
    ...
    userFilter: settings.ldapsUserFilter ?? config.ldap.userFilter, ca: settings.ldapsCaCertificate, cioGroupDn: settings.ldapsCioGroupDn ?? config.ldap.cioGroupDn,
  };
  ...
  try {
    ...
    return {
      ...
      isCio: ldap.cioGroupDn
        ? groups.some((group) => group.trim().toLowerCase() === ldap.cioGroupDn?.trim().toLowerCase())
        : undefined,
    };
  } finally {
...
export async function searchLdapsUsers(search = "") {
  ...
  try {
    ...
    return result.searchEntries.map((entry) => {
      ...
      return { subject: String(row.distinguishedName ?? row.dn ?? ""), email: String(row.mail ?? "").toLowerCase(), name: String(row.displayName ?? row.sAMAccountName ?? ""), authProvider: "ldaps" as const };
